...
function CalcSHA256(const Buf:
{$IFDEF LOCKBOXUNICODE}UnicodeString{$ELSE}AnsiString{$ENDIF}): T256BitDigest; overload;
...
function SHA256DigestToHex(var Digest: T256BitDigest): {$IFDEF LOCKBOXUNICODE}UnicodeString{$ELSE}AnsiString{$ENDIF};
{-------------------------------------------------------------------------------------------------}
implementation
...
function CalcSHA256(const Buf: {$IFDEF LOCKBOXUNICODE}UnicodeString{$ELSE}AnsiString{$ENDIF}): T256BitDigest;
begin
...
begin
  {$IFDEF LOCKBOXUNICODE}
  Result:=SHA256DigestToHexW(Digest);
  {$ELSE}
  Result:=SHA256DigestToHexA(Digest);
  {$ENDIF}
end;
{-------------------------------------------------------------------------------------------------}
end.
